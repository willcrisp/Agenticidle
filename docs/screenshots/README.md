...
| [`04-agents-running.png`](04-agents-running.png) | Three pods staffed and running. Note the pod payouts are already below what the board offered — `$2,819` → `$2,792` — and the credit bar has started to drain. The board is refilling behind you. |
...
One thing visible here is known-missing rather than done: there is no shop yet,
so `BUY MORE` is the placeholder single-block button and cash has nowhere to go
mid-run. Everything else on screen — the decay, the credit drain, the blocked
queue — is live sim state.
